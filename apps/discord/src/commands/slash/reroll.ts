...
export default class BotCommand extends SlashCommand {
  ...
  async run(ctx: CommandContext) {
    ...
    if (!(await state.ended)) {
      return ctx.send({
        content: "That giveaway is still running, so you can't reroll it right now. Try stopping it first.",
        ephemeral: true
      });
    }
    ...
    if (newWinners.length === 0) {
      return ctx.send({
        content: 'No new winners could be drawn, as there were no (other) entries.',
        ephemeral: true
      });
    }
    await ctx.send({
      content: `A new winner has been drawn! The new winner is <@${newWinners[0].user_id}>, congrats!`,
      allowedMentions: {
        everyone: false,
